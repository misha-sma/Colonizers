...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		frame.getContentPane().add(new DrawBackground());
		try {
			String imageFile = "test_image.png";
			Image image = ImageIO.read(new File(imageFile));
			System.out.println(frame.getContentPane().getClass());
			frame.getContentPane().add(new DragImage(image, 500, 0));
            frame.getContentPane().add(new DragImage(image, 500, 500));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}

	public static void drawField(Graphics g, int x, int y, int edgeSize) {
